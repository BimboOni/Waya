...
export function OtpInput({ onComplete, onResend, resendTimer = 0, isLoading }: OtpInputProps) {
  ...
  const handleInput = (e: React.FormEvent<HTMLInputElement>, index: number) => {
    ...
    if (last && index === 5) {
      const otp = [...next.slice(0, 5), last].join('');
      onComplete(otp);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <button
        ...
        className="w-full mt-5 min-h-[52px] px-10 py-3 rounded-full bg-brand-primary text-brand-on-primary font-body text-label-lg font-bold border-b-[5px] border-brand-hover transition-all duration-150 hover:brightness-110 active:translate-y-[2px] active:border-b-[1px] transition-all duration-100 disabled:opacity-30 disabled:cursor-not-allowed disabled:border-b-[5px] disabled:translate-y-0 flex items-center justify-center"
      >
        {isLoading ? (
