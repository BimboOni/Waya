        ...
        <div className="flex flex-col items-center text-center gap-4">
          ...
          <div className="flex gap-3 w-full mt-2">
            <button onClick={onClose}
              className="flex-1 bg-bg-card border-2 border-border-default border-b-4 text-text-secondary font-medium rounded-full h-12 px-6 active:translate-y-[2px] active:border-b-2 transition-all hover:brightness-95">
              Cancel
            </button>
            <button onClick={() => { onConfirm(); onClose(); }}
              className={`flex-1 font-medium rounded-full h-12 px-6 border-b-4 active:translate-y-[2px] active:border-b-2 transition-all hover:brightness-110 ${confirmClass ?? 'bg-bg-card border-2 border-error dark:border-red-900/50 border-b-4 border-b-red-800 text-error dark:text-red-300 hover:bg-error-container/50 hover:brightness-95'}`}>
              {confirmLabel}
            </button>
          </div>
        </div>
            ...
            {!showPasswordForm ? (
              <div className="flex flex-row justify-between items-center w-full">
                ...
                <button onClick={() => setShowPasswordForm(true)}
                  className="bg-bg-card border-2 border-border-default border-b-4 text-text-secondary font-medium rounded-full h-12 px-6 min-w-[180px] text-center active:translate-y-[2px] active:border-b-2 transition-all w-fit hover:brightness-95">
                  Change Password
                </button>
              </div>
            ) : (
                  ...
                  <button onClick={handleChangePassword} disabled={saving || !oldPassword || !passwordReqs.len || !passwordReqs.upper || !passwordReqs.lower || !passwordReqs.num || !passwordReqs.special || !confirmPassword}
                    className={`flex-1 rounded-full font-medium transition-all text-center h-12 px-6 ${
                      oldPassword && passwordReqs.len && passwordReqs.upper && passwordReqs.lower && passwordReqs.num && passwordReqs.special && confirmPassword
                        ? 'bg-brand-primary text-white border-b-4 border-brand-hover active:translate-y-[2px] active:border-b-2'
          ...
          <div className={sectionCard} style={{ border: sectionBorder }}>
            <div className="flex flex-row justify-between items-center w-full">
              ...
              <button onClick={() => setShowSignOutModal(true)}
                className="bg-bg-card border-2 border-error dark:border-red-900/50 border-b-4 border-b-red-800 text-error dark:text-red-300 font-medium rounded-full h-12 px-6 min-w-[180px] text-center active:translate-y-[2px] active:border-b-2 transition-all w-fit hover:bg-error-container/50 hover:brightness-95">
                  Sign Out
              </button>
            </div>
          </div>
